...
def main():
    ...
    if st.button('chill 모델 실행'):
        ...
        st.write(f'{select_year}년 {loc}의 복숭아의 예상 개화일은 {dvr_model(df, Tc, Hr, C, D)[0]}')
    ...
    if st.button('DVR 모델 실행'):
        ...
        df['DOY'] = df['date'].dt.dayofyear
        df.set_index('date', inplace=True)
        st.write(f'{select_year}년 {loc}의 복숭아의 예상 개화일은 {dvr_model(df, Tc, Hr, C, D)[1]}')
